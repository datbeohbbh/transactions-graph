...
func (bl *BlockListener) Tracking(ctx context.Context, address string) (bool, error) {
	return bl.graphDB.ExistedAddress(ctx, "tracking", address)
}

func (bl *BlockListener) InsertTrackedAddress(ctx context.Context, address, accountType string) (bool, error) {
	return bl.graphDB.InsertTrackedAddress(ctx, address, accountType)
}

func (bl *BlockListener) AddAddress(ctx context.Context, request *Address) (*AddResp, error) {
	...
	addStatus, err := bl.graphDB.InsertTrackedAddress(ctx, addr, accountType)
	if err != nil {
		return nil, err
	}
